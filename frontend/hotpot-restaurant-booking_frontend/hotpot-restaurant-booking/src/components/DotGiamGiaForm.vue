...
const gui = () => {
  ...
  if (loai !== loai.trim())
    return alert('Loại danh mục không được chứa khoảng trắng ở đầu hoặc cuối')
  if (/\s{2,}/.test(loai))
    return alert('Loại danh mục không được chứa nhiều khoảng trắng liên tiếp')
  ...
  emit('submit', { loaiDanhMuc: loai.trim(), moTa: mTa.trim() })
}
...
<template>
  <section class="bieu-mau-panel">
    ...
    <div class="luoi-bieu-mau">
      <label
        >Loại danh mục
        <input v-model="form.loaiDanhMuc" placeholder="Ví dụ: Món lẩu, Đồ uống..." />
      </label>
      <label
        >Mô tả
    ...
    <div class="nhom-nut">
      <button class="nut-chinh" @click="gui">Lưu</button>
    </div>
  </section>
</template>
